...
interface WorkflowRunListProps {
  ...
  agentNodeId?: string | null; // エージェントノードが選択されている場合
}

/**
 * ワークフロー実行履歴・成果物一覧（統合）
 * Step4改: HistoryとOutputを1つの一覧に統合
 */
export default function WorkflowRunList({ activeWorkflow, agentNodeId }: WorkflowRunListProps) {
  const { openRunDrawer } = useWorkflowStore();
  const [runs, setRuns] = useState<Array<{
    item: KBItem;
    payload: NormalizedWorkflowRunPayload;
    hasOutput: boolean; // finalOutput または parsedOutput がある
    agentDefinition?: any;
    planningEval?: ReturnType<typeof evaluateRunForPlanning>;
  }>>([]);
  const [loading, setLoading] = useState(false);
  const [filterMode, setFilterMode] = useState<'all' | 'outputs-only'>('all'); // 初期値は「すべて」
    ...
    try {
      ...
      const runsWithMetadataAndEval = await Promise.all(
        runsWithMetadata.map(async ({ item, payload }) => {
          ...
          // AgentDefinitionを取得（contract取得用）
      ...
      console.error('[WorkflowRunList] Failed to load runs:', error);
      // エラーメッセージを表示（開発環境のみ詳細表示）
      if (process.env.NODE_ENV === 'development') {
        console.error('[WorkflowRunList] Error details:', {
      ...
      setRuns([]);
      // エラーメッセージを設定
      const errorMessage = error.message || 'ワークフロー実行履歴の読み込みに失敗しました';
      setError(errorMessage);
    } finally {
      setLoading(false);
      loadingRef.current = false;
    }
  }, [activeWorkflow?.id, agentNodeId]);
  ...
  if (!activeWorkflow) {
    return (
      <div className="p-4">
        <h4 className="font-semibold mb-3">履歴・成果物</h4>
        <div className="text-sm text-gray-500">
          ワークフローを選択すると実行履歴が表示されます
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="font-semibold">履歴・成果物</h4>
        {/* トグル：成果物のみ / すべて */}
      ...
      {error ? (
        <div className="p-4 bg-red-50 border border-red-200 rounded">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              <div className="font-semibold text-red-800 mb-1">エラー: データの読み込みに失敗しました</div>
